...
| 32 | **purse** | 钱包（女式）| `[辨]` purse（英式：零钱包；美式：女式手提包/钱包）≠ wallet（男士钱包）；`[短语]` tighten the purse strings 紧缩开支 / hold the purse strings 掌管财务 / coin purse 零钱包；`[惯用]` the public purse 国库，公共资金 |
| 33 | **penny** | 便士；美分 | `[短语]` not a penny 一文不名 / a penny for your thoughts 你在想什么呢？/ every penny counts 每一分钱都很重要；`[惯用]` A penny saved is a penny earned. 省一分就是赚一分。（富兰克林名言）/ penny-wise, pound-foolish 小钱精明大钱糊涂；复数：pence（英，金额）/ pennies（美，硬币个数）|
| 34 | **disc** | 磁盘；圆盘 | `[辨]` disc（英式）= disk（美式）；`[短语]` hard disc 硬盘 / disc drive 磁盘驱动器 / compact disc (CD) 光盘 / slipped disc 椎间盘突出 |
| 35 | **trunk** | 树干；象鼻；行李箱 | `[辨]` trunk（美式：汽车后备箱）= boot（英式：汽车后备箱）；`[短语]` tree trunk 树干 / swimming trunks 泳裤 / trunk call 长途电话（英式旧用法）；`[记忆]` 一根树干 → 大象鼻子像树干 → 行李箱也像树干一样粗大 → trunk 三义同一词 |
| 36 | **vapour** | 蒸汽，水汽 | `[辨]` vapour（英式）= vapor（美式）；`[词根]` vap- = steam → vapour, evaporate（蒸发）, vapid（索然无味的，像蒸汽一样没味道）；`[短语]` water vapour 水蒸气 |
| 37 | **sift** | v. 筛；仔细审查 | `[短语]` sift through sth 仔细筛查 / sift flour 筛面粉 / sift out 筛除，筛选出；`[记忆]` sift 联想 sieve（筛子，名词）→ sift（筛的动作，动词）；`[惯用]` sift through piles of documents 在成堆的文件中仔细翻找 |
...
| 45 | **ape** | n. 猿 / v. 模仿 | `[辨]` ape（猿，无尾）≠ monkey（猴子，有尾）；`[短语]` go ape 发狂，暴跳如雷 / ape sb's style 模仿某人的风格；`[惯用]` "He went ape when he found out." 他发现后暴跳如雷。 |
| 46 | **mosquito** | 见 #5 | |
...
| 词根 | 含义 | 例词 |
|------|------|------|
| **quar-** | 四十 | quarantine, quarter, quartet, quart |
| **pneumon-** | 肺 | pneumonia, pneumatic, pneumonectomy |
| **pend-/pens-** | 悬挂；称量 | pendulum, depend, suspend, pending, pension |
| **ting-/tact-/tag-** | 接触，触摸 | contingent, contact, tangible, tactile, contagion |
| **spir-** | 呼吸 | conspiracy, conspire, spiral (间接), inspire, expire, spirit |
| **govern-/gubern-** | 掌舵，管理 | governor, govern, government, governance, gubernatorial |
| **leg-/legis-** | 法律 | legitimate, legislature, legal, legislate, illegal, privilege |
| **inter-** | 之间，相互 | intervention, international, interact, intervene, interrupt |
| **ven-/vent-** | 来 | intervention, convene, prevent, event, advent, venture |
| **clud-/clus-/clos-** | 关闭 | enclose, include, exclude, conclude, disclose, closet |
| **trud-/trus-** | 推，插入 | intrude, extrude, protrude, intrusion, abstruse |
| **cess-/ced-/ceed-** | 走，前进 | procession, process, proceed, succeed, exceed, access, recess |
| **lucr-** | 利润，钱财 | lucrative, lucre |
| **plic-/ply-/plex-** | 折叠 | explicatory, explicit, implicit, complicate, complex, reply |
| **vap-** | 蒸汽 | vapour, evaporate, vapid, vaporize |
| **pend-** | 悬挂 | pendulum, depend, suspend, append, impending |
...
| 词对 | 区别 |
|------|------|
| **contingent** vs **continent** | contingent = 取决于…的 / 代表团；continent = 大陆 |
| **governor** vs **government** | governor = 州长/管理者（人）；government = 政府（机构） |
| **legislature** vs **judiciary** vs **executive** | legislature = 立法机关；judiciary = 司法机关；executive = 行政机关 |
| **legitimate** vs **legal** vs **lawful** | legitimate = 合法+正统（强调正当性）；legal = 法律上的（偏技术）；lawful = 依法的（偏古雅/正式） |
| **cemetery** vs **graveyard** vs **tomb** | cemetery = 公墓（非教堂所属）；graveyard = 教堂墓地；tomb = 坟墓单体 |
| **naked** vs **nude** vs **bare** | naked = 裸体（通用，也比喻赤裸裸的）；nude = 裸体（艺术/摄影）；bare = 光秃的（bare foot 赤脚） |
| **jug** vs **jar** vs **mug** vs **pitcher** | jug = 带柄和嘴的壶（英式 pitcher）；jar = 广口罐（储物）；mug = 杯子（喝水）；pitcher = 大水罐（美式）= jug（英式） |
| **purse** vs **wallet** | purse = 女式钱包/零钱包（英式）；wallet = 男士钱包 |
| **disc** vs **disk** | disc（英式）= disk（美式） |
| **vapour** vs **vapor** | vapour（英式）= vapor（美式） |
| **trunk** vs **boot** | trunk = 汽车后备箱（美式）；boot = 汽车后备箱（英式） |
| **ox** vs **bull** vs **cow** vs **cattle** | ox = 阉公牛（干活）；bull = 未阉公牛（斗牛）；cow = 母牛/奶牛；cattle = 牛的总称（不可数集合名词，谓语用复数） |
| **ape** vs **monkey** | ape = 猿（无尾）；monkey = 猴子（有尾） |
| **shrub** vs **bush** vs **tree** vs **herb** | shrub = 灌木（单株）；bush = 灌木丛；tree = 树（有主干）；herb = 草本植物 |
| **cock** vs **rooster** | cock = 公鸡（英式，口语另有粗俗义慎用）；rooster = 公鸡（美式，安全通用） |
| **explicatory** vs **explanatory** | explicatory = 解释的（正式/学术）= explanatory（更常用） |
| **procession** vs **parade** | procession = 队列（送葬/仪式）；parade = 游行/阅兵（庆祝） |
| **soil** vs **dirty** | soil = 弄脏（正式/文学）；dirty = 弄脏（口语通用） |
| **encounter** vs **meet** | encounter = 遭遇/邂逅（较正式，含意外意味）；meet = 遇见（最通用） |
...
| 考点 | 说明 |
|------|------|
| **cattle 的谓一致** | cattle 不可数但谓语必用复数！例：Cattle **are** raised here. |
| **trunk 的英美差异** | 汽车后备箱：trunk（美）= boot（英）；泳裤：swimming trunks |
| **legislature ≠ judiciary** | 三权分立：legislature（立法）/ executive（行政）/ judiciary（司法） |
