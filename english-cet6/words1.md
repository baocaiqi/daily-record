# 📚 CET-6 单词本

> 标注说明：`[词根]` = 词根/词缀注释 | `[短语]` = 固定搭配 | `[近义]` = 近义词群

| # | 单词/短语 | 释义 | 备注 |
|---|-----------|------|------|
| 1 | **queer** | 奇怪的；同性恋者 | |
| 2 | **affluent** | 富裕的 | |
| 3 | **freelance** | 自由职业（者） | |
| 4 | **cardinal** | 基本的；基数的 | `[词根]` cardi- = heart（心脏） |
| 5 | **card(i)-** 🏷️ | 心脏 | `[词根]` 衍生：**card**iac 心脏的 / **card**iology 心脏病学 |
| 6 | **saline** | 盐的，含盐的 | 疑原为 saline，与下条 sal- 同源 |
| 7 | **sal-** 🏷️ | 盐 | `[词根]` 衍生：**sal**inity 盐度 / **sal**ary 薪水（古罗马以盐为薪） |
| 8 | **superstition** | 迷信 | |
| 9 | **manoeuvre** | 策略；调动；演习 | = maneuver（美式） |
| 10 | **soar** | 翱翔；猛增 | |
| 11 | **spiral** | 螺旋（的） | |
| 12 | **panorama** | 全景 | pan-（全）+ horama（景象） |
| 13 | **hypocrisy** | 虚伪，伪善 | |
| 14 | **conspiracy** | 阴谋，共谋 | `[词根]` spir- = breathe（con- 共同 + spire 呼吸 → 一个鼻孔出气） |
| 15 | **allegiance to** | 忠诚于，效忠于 | `[短语]` |
| 16 | **custody** | 监护；拘留 | in custody 被拘留 |
| 17 | **the advent of** | …的到来，…的出现 | `[短语]` |
| 18 | **lapse** | （时间）流逝；失误 | time lapse 时间流逝 / a lapse of judgment 判断失误 |
| 19 | **herald** | 预示…的到来；先驱 | |
| 20 | **encyclopedia** | 百科全书 | |
| 21 | **choir** | 合唱团 | |
| 22 | **aesthetic** | 审美的，美学的 | ✏️ 原拼写 `aesthestic` 修正 |
| 23 | **a different kettle of fish** | 另一码事，完全不同的事 | `[短语]` |
| 24 | **chalk sth up to sth** | 把…归因于… | `[短语]` |
| 25 | **slip** | 滑倒；失误 | `[近义]` → #27–#30 错误系列 |
| 26 | **gaffe** | 失态，失言 | ✏️ 原拼写 `gaffle` 推测修正 |
| 27 | **fault** | 错误；缺点；过错 | `[近义]` |
| 28 | **blunder** | 大错，愚蠢的错误 | `[近义]` 比 mistake 语气更重 |
| 29 | **error** | 错误，误差 | `[近义]` 偏正式/技术语境 |
| 30 | **mistake** | 错误，误会 | `[近义]` 最通用 |
| 31 | **robe** | 长袍，浴袍 | |
| 32 | **abide by** | 遵守（规则/法律） | `[短语]` |
| 33 | **shorthand** | 速记（法） | |
| 34 | **retention** | 保留；记忆力 | `[词根]` tent-/ten- = hold |
| 35 | **avail you nothing** | 对你毫无用处 | `[短语]` |
| 36 | **flame** | 火焰 | 🔥 火焰系列 ↓ |
| 37 | **flare** | 闪耀；突然爆发 | |
| 38 | **scorch** | 烧焦，烤焦 | |
| 39 | **char** | 烧焦，炭化 | |
| 40 | **blaze** | 熊熊燃烧；火焰 | |
| 41 | **kindle** | 点燃；激起 | Kindle 阅读器（点燃知识之火） |
| 42 | **ignite** | 点燃；引发 | |
| 43 | **dissipate** | 消散，消失；挥霍 | |
| 44 | **strife** | 冲突，争斗 | ✏️ 原拼写 `strif` 修正 |
| 45 | **furnace** | 熔炉，炉子 | |
| 46 | **overhaul** | 彻底检修；全面改革 | |
| 47 | **archaeology** | 考古学 | `[词根]` archaeo- = ancient（古代的） |

---

### 📌 词根小结

| 词根 | 含义 | 例词 |
|------|------|------|
| **cardi-** | 心脏 | cardiac, cardiology, cardiogram |
| **sal-** | 盐 | saline, salinity, salary |
| **spir-** | 呼吸 | conspiracy, inspire, expire, spirit |
| **tent-/ten-** | 持有 | retention, content, maintain |
| **archaeo-** | 古代的 | archaeology, archaic |
